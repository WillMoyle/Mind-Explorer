...
DiskStorageManager::DiskStorageManager(Tools::PropertySet& ps) : m_pageSize(0), m_nextPage(-1), m_buffer(0)
{
	...
	if (var.m_varType != Tools::VT_EMPTY)
	{
        ...
        CFURLRef datURL = CFBundleCopyResourceURL(mainBundle, CFSTR("FLATindex_index"), CFSTR("dat"), NULL);
        ...
        std::string sDataFile = CFStringGetCStringPtr(datPath, encodingMethod);
        
        // Get a reference to the file's URL
        CFURLRef idxURL = CFBundleCopyResourceURL(mainBundle, CFSTR("FLATindex_index"), CFSTR("idx"), NULL);
        
        // Convert the URL reference into a string reference
        CFStringRef idxPath = CFURLCopyFileSystemPath(idxURL, kCFURLPOSIXPathStyle);
        
        // Convert the string reference into a C string
        std::string sIndexFile = CFStringGetCStringPtr(idxPath, encodingMethod);*/
		...
		else
		{
			...
			if (m_dataFile.fail())
				throw Tools::IllegalArgumentException("SpatialIndex::DiskStorageManager: Data file cannot be created.");
            
            if (m_indexFile.fail())
                throw Tools::IllegalArgumentException("SpatialIndex::DiskStorageManager: Index file cannot be created.");
